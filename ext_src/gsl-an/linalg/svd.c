...
//     size_t rows = m->size1; // 矩阵的行数
//     for (size_t row = 0; row < rows; row++) {
//         // 交换第 row 行的第 i 列和第 j 列的元素
//         double temp = gsl_matrix_get(m, row, i);
//         gsl_matrix_set(m, row, i, gsl_matrix_get(m, row, j));
//         gsl_matrix_set(m, row, j, temp);
//     }
