...
static uint8_t* config_file_data = NULL; // 用于共享文件数据
static int config_file_size = 0;         // 用于共享文件大小
FILE *configfile = NULL;

// 定义一个函数来处理文件内容
EMSCRIPTEN_KEEPALIVE
void process_config_file(uint8_t* file_data, int file_size) {
    printf("File size: %d\n", file_size);

    config_file_data = file_data; // 共享文件数据指针
    config_file_size = file_size;  // 共享文件大小

    // 将shared_file_data写入sharefile
    configfile = fopen("my_astrometry.cfg", "wb"); // 打开文件以进行写入，您可以根据需要更改文件名
    if (configfile) {
        fwrite(config_file_data, sizeof(uint8_t), config_file_size, configfile); // 写入数据
        fclose(configfile); // 关闭文件
    } else {
...
void run_engine_main(int argc, char** args){
    logverb("run_engine_main--------------- 0\n");
