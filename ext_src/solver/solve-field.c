...
extern void run_engine_main(int argc, char** args);
...
static void run_engine(sl* engineargs) {
    ...
    fflush(NULL);
    // 构造参数
    const char* argv[] = {"program_name", "--verbose", "a.axy", NULL};
    int argc = sizeof(argv) / sizeof(argv[0]) - 1;

    // 调用 run_engine_main
    run_engine_main(argc, (char**)argv);
...
void core_test2(int numStrings, char** strings) {
    printf("Number of strings: %d\n", numStrings);
    for (int i = 0; i < numStrings; i++) {
        if (strings[i] != NULL) {
            printf("String %d: %s\n", i, strings[i]);
        } else {
            printf("String %d: NULL\n", i);
        }
    }
}
...
EMSCRIPTEN_KEEPALIVE
void run_solve_field(int argc, char** args) {

    // 打印参数数量
    printf("Running solve_field with %d arguments:\n", argc);
    ...
    while (1) {
        ...
        //logmsg("Base directory: %s\n", basedir);

        asprintf_safe(&base, "%s/%s", basedir, basefile);
        //logmsg("Base name for output files: %s\n", base);
        ...
        logmsg("Base: \"%s\", basefile \"%s\", basedir \"%s\", suffix \"%s\"\n", base, basefile, basedir, suffix);
    ...
    EM_ASM({
        console.log("This is solve-field **********5");
    });
}
