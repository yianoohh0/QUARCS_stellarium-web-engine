...
//static uint8_t* shared_file_data = NULL; // 用于共享文件数据
static int shared_file_size = 0;         // 用于共享文件大小
FILE *sharefile = NULL;

// 定义一个函数来处理文件内容
EMSCRIPTEN_KEEPALIVE
void process_file(uint8_t* file_data, int file_size) {
    uint8_t* shared_file_data = NULL;
    printf("File size: %d\n", file_size);

    shared_file_data = file_data; // 共享文件数据指针
    shared_file_size = file_size;  // 共享文件大小


    // 将shared_file_data写入sharefile
    sharefile = fopen("output_file.bin", "wb"); // 打开文件以进行写入，您可以根据需要更改文件名
    if (sharefile) {
        fwrite(shared_file_data, sizeof(uint8_t), shared_file_size, sharefile); // 写入数据
        fclose(sharefile); // 关闭文件
    } else {
        perror("Failed to open sharefile for writing"); // 错误处理
    }
...
int augment_xylist(augment_xylist_t* axy,
    ...
    if ((axy->scalelo > 0.0) || (axy->scalehi > 0.0)) {
        ...
        case SCALE_UNITS_ARCMIN_WIDTH:
            ...
            appl = (axy->scalelo * 60.0) / (double)axy->W;
            appu = (axy->scalehi * 60.0) / (double)axy->W;
            logverb("Image width %i pixels; arcsec per pixel range %g %g\n", axy->W, appl, appu);
            break;
