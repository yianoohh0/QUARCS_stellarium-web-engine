...
static uint8_t* index_file_data = NULL; // 用于共享文件数据
static int index_file_size = 0;         // 用于共享文件大小


// 定义一个函数来处理文件内容
EMSCRIPTEN_KEEPALIVE
void process_index_file(uint8_t* file_data, int file_size, const char* file_name) {
    printf("Processing file: %s\n", file_name);
    printf("File size: %d\n", file_size);

    index_file_data = file_data; // 共享文件数据指针
    index_file_size = file_size;  // 共享文件大小

    // 将shared_file_data写入sharefile
    FILE *indexfile = fopen(file_name, "wb"); // 打开文件以进行写入，您可以根据需要更改文件名
    if (indexfile) {
        fwrite(index_file_data, sizeof(uint8_t), index_file_size, indexfile); // 写入数据
        fclose(indexfile); // 关闭文件
    } else {
        perror("Failed to open configfile for writing"); // 错误处理
    }
...
int engine_autoindex_search_paths(engine_t* engine) {
    ...
    for (i=0; i<sl_size(engine->index_paths); i++) {
        ...
        char* path = "/usr/local/astrometry/data";
...
int engine_parse_config_file_stream(engine_t* engine, FILE* fconf) {
    ...
    if (auto_index) {
        printf("----------engine_autoindex_search_paths  0\n");
...
int engine_run_job(engine_t* engine, job_t* job) {
    ...
    app_min_default = (engine->minwidth * 3600.0) / job_imagew(job);
    app_max_default = (engine->maxwidth * 3600.0) / job_imagew(job);
...
job_t* engine_read_job_file(engine_t* engine, const char* jobfn) {
    ...
    if (!dl_size(job->scales) || job->include_default_scales) {
        ...
        arcsecperpix = (engine->minwidth * 3600.0) / job_imagew(job);
        dl_append(job->scales, arcsecperpix);
        // arcsecperpix = deg2arcsec(engine->maxwidth) / job_imagew(job);
        arcsecperpix = (engine->maxwidth * 3600.0) / job_imagew(job);
        dl_append(job->scales, arcsecperpix);
    }
